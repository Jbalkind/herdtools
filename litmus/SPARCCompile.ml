...
(* Memory *)
    let ld r1 r2 r3 =
      { empty_ins with
        memo = sprintf "ld [^i0+^i1],^o0";
        inputs = [r1;r2] ;
        outputs = [r3] ; }

    let ldi r1 kr r3 =
      { empty_ins with
        memo = sprintf "ld [^i0+%i],^o0" kr ;
        inputs = [r1] ;
        outputs = [r3] ; }

    let st r1 r2 r3 =
      { empty_ins with
        memo = sprintf "st ^i0,[^i1+^i2]";
        inputs = [r1;r2;r3;] ;
        outputs = []; }

    let sti r1 r2 kr =
      { empty_ins with
        memo = sprintf "st ^i0,[^i1+%i]" kr ;
        inputs = [r1;r2;] ;
        outputs = []; }
...
(* Branch *)
    ...
    | LD (r1,A.RV r2,r3) -> ld r1 r2 r3::k
    | LD (r1,A.K kr,r3) -> ldi r1 kr r3::k
    | ST (r1,r2,A.RV r3) -> st r1 r2 r3::k
    | ST (r1,r2,A.K kr) -> sti r1 r2 kr::k
